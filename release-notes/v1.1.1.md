...
## The addon didn't run on the Godot version it advertises

The badge says 4.3+. Three constructs said otherwise: a typed `Dictionary` (4.4+),
`OS.get_temp_dir()` (4.5+), and an inferred return from
`EditorInterface.close_scene()`. On 4.3 those are parse errors, which means the
plugin would not load at all.

CI has been pinned at 4.3 the whole time and stayed green, because the old check
called `load()` — too weak to catch it. Compiling the source found it on the first
run. All three now have 4.3-compatible equivalents, and CI pinned at the claimed
minimum is what keeps the badge checkable rather than aspirational.
