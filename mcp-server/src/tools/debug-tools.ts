...
export const debugTools: ToolDefinition[] = [
  {
    ...
    description: 'Start a debug session: launch the project (or a specific scene) under Godot\'s debug adapter. Set breakpoints FIRST with debug_set_breakpoints — they are buffered and applied during the launch handshake. Returns the session state; if a breakpoint is hit immediately, state is "stopped". IF EXECUTION NEVER PAUSES at a breakpoint that is reported verified, the cause is almost always the editor\'s "Skip Breakpoints" toggle (Debugger panel, top-left, a crossed-out breakpoint icon): it is persisted editor state and it suppresses every breakpoint from every source. Check that before assuming the breakpoint is wrong.',
    inputSchema: {
      type: 'object',
      properties: {
