 ...
 * this test can collide with a real godot-mcp-bridge already running on
 ...
 * real godot-mcp-bridge (e.g. the user's own editor session) already owns
...
describe.skipIf(canRun)('E2E — real Godot editor process (skipped)', () => {
  it('skips: Godot binary missing or port 6505 already in use', () => {
    if (!godotAvailable) {
      ...
      console.log(`[e2e-godot] Skipped: port ${BRIDGE_PORT} is already in use by another process (likely a real godot-mcp-bridge).`);
    }
